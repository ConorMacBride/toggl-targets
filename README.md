...
### Download files
Download `targets.py`, `config.csv` and `projects.py` to your computer. Make sure `targets.py` and `config.csv` are kept in the same folder.
...
You need to determine the project IDs of all the projects that are being tracked. The included `projects.py` script can be used for this. Run `python3 project.py API_TOKEN` replacing `API_TOKEN` with your API token from before. Select your workspace and then make a note of the project IDs that you want to track.
